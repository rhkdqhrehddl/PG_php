<?php
    ...
    $LGD_NOINT                    = $_POST["LGD_NOINT"];				// 무이자여부
    ...
    $LGD_USE_INTENT_ONLY                    = $_POST["LGD_USE_INTENT_ONLY"];					// ISP/계좌이체관련 앱 마켓으로 이동
    $LGD_CANCELURL                    = $_POST["LGD_CANCELURL"];					// 안심클릭 취소 URL
    $LGD_SELF_CUSTOM                    = $_POST["LGD_SELF_CUSTOM"];						// 신용카드선택페이지 스킵관련 파라미터
     ...
     */    
    $LGD_CASNOTEURL				= "https://" . $server_domain . "/CardMobileXPay/cas_noteurl.php";    

    /*
     * LGD_RETURNURL 을 설정하여 주시기 바랍니다. 반드시 현재 페이지와 동일한 프로트콜 및  호스트이어야 합니다. 아래 부분을 반드시 수정하십시요.
     */    
    $LGD_RETURNURL				= "https://" . $server_domain . "/CardMobileXPay/returnurl.php";     
	...
	*/
	...
	if($LGD_CUSTOM_FIRSTPAY == 'SC0010'){
		...
		$payReqMap['LGD_SP_CHAIN_CODE']                = $LGD_SP_CHAIN_CODE;					//간편결제사용여부
		$payReqMap['LGD_SP_ORDER_USER_ID']                = $LGD_SP_ORDER_USER_ID;				//간편결제 쇼핑몰 KEY_ID(계약된 일부 상점만 가능)
		$payReqMap['LGD_USE_INTENT_ONLY']                = $LGD_USE_INTENT_ONLY;						// ISP/계좌이체관련 앱 마켓으로 이동
		//$payReqMap['LGD_CANCELURL']                = $LGD_CANCELURL;					// 안심클릭 취소 URL
		$payReqMap['LGD_SELF_CUSTOM']                = $LGD_SELF_CUSTOM;				// 신용카드선택페이지 스킵관련 파라미터
	}
