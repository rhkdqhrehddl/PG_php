...
//정상적으로 처리한 경우에도 토스페이먼츠에서 응답을 받지 못한 경우는 결제결과가 중복해서 나갈 수 있으므로 관련한 처리도 고려되어야 합니다. 
...
//PHP에서는 되도록이면 error_reporting() 함수를 이용하여 개발 후에는 단순한 경고메세지는 출력이 되지 않도록 해주십시요.
	...
	$mid="";					// 상점아이디 
	$tid="";					// 토스페이먼츠에서 부여한 거래번호
	$oid="";					// 상품번호
	$ssn = "";					// 구매자주민번호
	$ip = "";					// 구매자IP
	$mac = "";					// 구매자 mac
	$hashdata = "";				// 토스페이먼츠 인증 데이터
	$productid = "";			// 상품정보키
	$resdate = "";				// 구매확인 요청일시
    ...
    $mertkey = ""; //토스페이먼츠에서 발급한 상점키로 변경해 주시기 바랍니다.
