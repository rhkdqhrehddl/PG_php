<?php
    ...
    $LGD_INSTALL                = $_POST["LGD_INSTALL"];	 					//할부개월수, 파라미터만 넘김
    $LGD_EXPYEAR	            = $_POST["LGD_EXPYEAR"]; 						//유효기간년, 파라미터만 넘김
    $LGD_EXPMON	                = $_POST["LGD_EXPMON"];		 					//유효기간월 , 파라미터만 넘김
	$LGD_PIN                	= $_POST["LGD_PIN"];	 						//비밀번호 앞2자리(옵션-주민번호를 넘기지 않으면 비밀번호도 체크 안함), 파라미터만 넘김
    $LGD_PRIVATENO	            = $_POST["LGD_PRIVATENO"]; 						//생년월일 6자리 (YYMMDD) or 사업자번호(옵션)
    $LGD_PRODUCTINFO            = $_POST["LGD_PRODUCTINFO"];                    //상품명
    $LGD_BUYER                  = $_POST["LGD_BUYER"];                          //고객명
    $LGD_BUYERID                = $_POST["LGD_BUYERID"];                        //고객 아이디
